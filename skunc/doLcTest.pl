...
sub doLcTest {
my($colNames, $table, $distinctUsers, $ofh, $seed) = @_;

  print "setting seed to $seed\n";
  random_set_seed_from_phrase($seed);
  print $ofh "testNum,numMatchingUsers,numMatchingColumns,columnMask,attrFreq\n";
  print "testNum,numMatchingUsers,numMatchingColumns,columnMask,attrFreq\n";
  ...
  while(1) {
    ...
    if ($foundMatch == 0) {
      # need to report that no match was found
      print $ofh "$totTries,0,0,\"0\",$freqStr,\"$vAttrName\",\"$vAttrVal\"\n";
      print "$totTries,0,0,\"0\",$freqStr,\"$vAttrName\",\"$vAttrVal\"\n";
    }
